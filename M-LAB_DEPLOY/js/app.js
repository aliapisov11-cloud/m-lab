...
async function testGeminiApiKey(keyToTest) {
  ...
  if (cleanKey.startsWith("ghp_") || cleanKey.startsWith("github_pat_")) {
    AppState.aiCamera.apiKeyStatus = "invalid";
    AppState.aiCamera.apiKeyError = "Siz GitHub tokeni kiritdingiz (" + cleanKey.substring(0, 8) + "...). Matematikani yechish uchun Google AI Studio ('AIzaSy...') kaliti kerak.";
  ...
  if (!cleanKey.startsWith("AIzaSy")) {
    AppState.aiCamera.apiKeyStatus = "invalid";
    AppState.aiCamera.apiKeyError = "Google AI Studio kaliti odatda 'AIzaSy' bilan boshlanadi. Kalitni to'g'ri ko'chirganingizga ishonch hosil qiling.";
    renderCameraModalContent();
    return false;
  }
  ...
  try {
    let res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${cleanKey}`, {
      method: "POST",
    ...
    if (!res.ok) {
      res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${cleanKey}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(pingBody)
      });
    ...
    if (res.ok) {
      ...
      showToast("✅ Google AI Studio API kaliti muvaffaqiyatli ulandi va tekshirildi!", "success");
      renderCameraModalContent();
      return true;
    } else {
      const errJson = await res.json().catch(() => ({}));
      const errorMsg = errJson.error?.message || `Google API javobi: HTTP ${res.status}`;
      AppState.aiCamera.apiKeyStatus = "invalid";
      AppState.aiCamera.apiKeyError = errorMsg;
      showToast(`❌ Kalitda xatolik: ${errorMsg}`, "error");
      renderCameraModalContent();
      return false;
    }
  } catch (netErr) {
    AppState.aiCamera.apiKeyStatus = "invalid";
    AppState.aiCamera.apiKeyError = "Internet aloqasi yoki Google API serveri bilan bog'lanib bo'lmadi.";
...
function renderCameraModalContent() {
  ...
  const isKeyValid = cam.apiKeyStatus === "valid" || (rawKey.startsWith("AIzaSy") && cam.apiKeyStatus !== "invalid");
  let contentHtml = "";
