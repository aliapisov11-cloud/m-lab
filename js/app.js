...
4. Javobni FAQAT quyidagi JSON formatida qaytaring:
{
  ...
  "verification": "Javobning qisqacha xulosasi"
    ...
    try {
      ...
      let response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${rawKey}`, {
        method: "POST",
      ...
      if (!response.ok) {
        response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${rawKey}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody)
        });
  ...
  if (cam.isAnalyzing) {
    ...
    contentHtml = `
      <div class="space-y-6 animate-fadeIn">
        ...
        <div class="flex items-center justify-between text-[11px] text-slate-400 dark:text-slate-500 pt-1 border-t border-slate-200/60 dark:border-slate-800">
          <span class="flex items-center space-x-1">
            <span>✨ M-LAB Aqlli Matematika Dvigateli & Gemini Vision</span>
          </span>
          <button id="ai-toggle-settings-btn" class="hover:text-brand-600 dark:hover:text-brand-400 flex items-center space-x-1 font-bold">
            ...
            <span>Google API Kaliti sozlamasi</span>
          </button>
        </div>
        ...
        <div id="ai-settings-panel" class="${cam.showSettings ? '' : 'hidden'} p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-3">
          <div class="flex items-center justify-between">
            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">
              🔑 Google Gemini API Kaliti (AI Studio):
            </label>
          ...
          <p class="text-[10px] text-slate-500 dark:text-slate-400">
            💡 Google AI Studio kaliti <code>AIzaSy...</code> bilan boshlanadi va 100% bepul beriladi. Agar kalit kiritilmasa, M-LAB o'rnatilgan aqlli dvigateli orqali avtomatik to'liq yechib beradi.
          </p>
        </div>
      </div>
    `;
